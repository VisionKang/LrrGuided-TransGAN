...
def crop_to_patch(img, stride):
    h = img.shape[0]
    w = img.shape[1]
    print(h)
    print(w)
    all_img = []
    for i in range(0, h - 224, stride):
        for j in range(0, w - 224, stride):
            img_patch = img[i:i + 224, j:j + 224].reshape(1, 224, 224)
            all_img.append(img_patch)
            if i + 224 >= h:
                img_patch = img[h - 224:, j:j + 224].reshape(1, 224, 224)
                all_img.append(img_patch)
        img_patch = img[i:i + 224, w - 224:].reshape(1, 224, 224)
        all_img.append(img_patch)
    all_img = np.stack(all_img, axis=0).reshape(len(all_img), 1, 224, 224)  # (b, 1, 224, 224)
    return all_img
...
def get_train_images_auto(paths):
    ...
    for image in images:
        crop_image = crop_to_patch(image, 150)
        crop_images.append(crop_image)

    crop_images = np.concatenate(crop_images, axis=0)
    crop_images = torch.from_numpy(crop_images).float()
    crop_images = (crop_images-127.5)/127.5
...
def get_img_parts8(image, h, w):
    pad = nn.ConstantPad2d(padding=(0, 672-w, 0, 0), value=0)
    image = torch.from_numpy(image)
    image = pad(image)
    images = []
    img1 = image[0:224, 0: 224]
